...
echo -e "\e[33m[NGSPIPE] Fastuniq reads \e[39m"
zcat $FQ1 > $FQ.R1.fq
zcat $FQ2 > $FQ.R2.fq
...
fastuniq -t q -i $FQ.list.txt \
-o 01.$FQ.R1.fq \
-p 01.$FQ.R2.fq

pigz -p $NT 01.$FQ.R1.fq 
pigz -p $NT 01.$FQ.R2.fq 
rm $FQ.R1.fq $FQ.R2.fq $FQ.list.txt
...
parallel -j $NT "
	...
	gatk3 -T RealignerTargetCreator -R $FASTAGENOME -I 03.$FQ.bam -o $FQ.chr{}.intervals -L chr{}
	# local realignment
	gatk3 -T IndelRealigner -R $FASTAGENOME -I 03.$FQ.bam -targetIntervals $FQ.chr{}.intervals -L chr{} -o $FQ.chr{}.realigned.bam
" ::: {1..22} X Y M
